...
class LSCondition:
    ...
    def condition_display(self, color):
        ...
        self.center_x = (256 + (256 / 2) - (self.show_c.get_width() / 2) - 15)
        self.screen.master.blit(self.show_c, (self.center_x, 520))

    def condition_light(self):
        self.condition_display(cst.WHITE)
        self.condition_pics(cst.SUN, cst.FEW_CLOUDY_SUN, cst.CLOUDY_DAY,
                            cst.LIGHT_RAIN_DAY, cst.STORM_DAY)

    def condition_dark(self):
        self.condition_display(cst.GREY)
        self.condition_pics(cst.MOON, cst.FEW_CLOUDY_MOON, cst.CLOUDY_NIGHT,
                            cst.LIGHT_RAIN_NIGHT, cst.STORM_NIGHT)

    def condition_pics(self, mode1, mode2, mode3, mode4, mode5):
